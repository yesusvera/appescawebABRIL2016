...
public class FormularioService {
	...
	/**
	 * 
	 * Duas letras do Estado (AP, PA e MA) Quatro dígitos (inicia em 0000) Sigla
	 * do recurso pesqueiro (Exemplo.: CR – Camarão Regional)
	 * 
	 * @param formulario
	 * @return
	 */
	...
	public Resposta getResposta(String chave, Formulario formulario) {
		...
		try {
			...
			if (indices.length == 3) {
				...
				Resposta resp = getResposta(new Integer(q), new Integer(p), new Integer(r), formulario);
	...
	public Pergunta getPergunta(int ordemQuestao, int ordemPergunta, Formulario formulario) {
		Questao questao = getQuestaoPorOrdem(ordemQuestao, formulario);
		Pergunta pergunta = getPerguntaPorOrdem(ordemPergunta, questao);
	...
	public boolean temDegravacao(Formulario formulario) {
		// Formulario formulario = formularioService.findById(new
